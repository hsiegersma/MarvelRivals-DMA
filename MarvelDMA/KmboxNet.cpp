...
int InitDevice(const std::string& IP, WORD Port, const std::string& Mac)
{
	...
	tx.head.mac = std::stoll(Mac, nullptr, 16);
	tx.head.rand = rand();
	tx.head.indexpts = 0;
	tx.head.cmd = cmd_connect;

	Status = sendto(sockClientfd, reinterpret_cast<const char*>(&tx), sizeof(cmd_head_t), 0,
		reinterpret_cast<sockaddr*>(&addrSrv), sizeof(addrSrv));
	std::this_thread::sleep_for(std::chrono::milliseconds(20));

	int FromLen = sizeof(addrSrv);

	Status = recvfrom(sockClientfd, reinterpret_cast<char*>(&rx), 1024, 0,
		reinterpret_cast<sockaddr*>(&addrSrv), &FromLen);
	if (Status < 0)
		return err_net_rx_timeout;
